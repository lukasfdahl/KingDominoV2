...
def main():
    ...
    print("\nCluster Centers (Each row represents a cluster centroid):")
...
def get_terrain(tile, x, y, file_number):
    ...
    # Print tile features
    print(f"\n--- Tile ({x}, {y}) ---")
    print(f"Dominant Hue Bin: {dominant_hue_bin}")
    print("Gradient Magnitude Histogram:")
    print(np.array2string(magnitude_hist, precision=4, separator=', '))
    print("Gradient Orientation Histogram:")
    print(np.array2string(orientation_hist, precision=4, separator=', '))
    print("Hue Histogram:")
    print(np.array2string(hue_hist, precision=4, separator=', '))
    print("----------------------------")
